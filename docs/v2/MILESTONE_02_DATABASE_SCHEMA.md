...
Define the complete Phase 1 database schema using Drizzle ORM, covering 15 tables and 7 enums. The schema must include proper foreign key constraints, CHECK constraints, and indexes from day one to avoid retroactive migrations. This is a ground-up rewrite — the schema incorporates V2 improvements (e.g., `reservedStock` on products) while remaining compatible with data ported from the V1 Neon PostgreSQL database.
...
| `src/server/db/index.ts` | Database connection pool and Drizzle instance |
| `src/server/db/schema.ts` | Complete Drizzle schema (15 tables, 7 enums, relations) |
| `src/shared/types.ts` | Re-exported inferred types for use across server and client |
...
# Expected: creates drizzle/XXXX_*.sql migration file

# 2. Inspect generated SQL for CHECK constraints
grep -i "CHECK" drizzle/*.sql
# Expected: chk_current_stock_non_negative, chk_reserved_stock_non_negative,
#           chk_reserved_lte_current, chk_quantity_positive,
#           chk_shipped_quantity_non_negative

# 3. Inspect generated SQL for indexes
grep -i "CREATE INDEX" drizzle/*.sql
# Expected: idx_products_sku, idx_products_category_id, idx_products_barcode,
#           idx_orders_status, idx_orders_customer_id, idx_order_items_order_id,
#           idx_order_items_product_id, idx_inventory_changes_product_id,
#           idx_inventory_changes_timestamp

# 4. Run migration against the database
npx drizzle-kit migrate
# Expected: migration applied successfully

# 5. Verify tables exist
docker compose exec postgres psql -U amphoreus -d amphoreus -c "\dt"
# Expected: 15 tables listed

# 6. Verify enums exist
docker compose exec postgres psql -U amphoreus -d amphoreus -c "\dT+"
# Expected: 7 custom enum types

# 7. TypeScript compiles
npx tsc --noEmit
# Expected: zero errors

# 8. Test DB connection from application code
npx tsx -e "
  import { db } from './src/server/db/index.js';
  import { users } from './src/server/db/schema.js';
  const result = await db.select().from(users).limit(1);
  console.log('DB connection OK, users:', result.length);
  process.exit(0);
"
```
...
- [ ] `schema.ts` defines all 15 tables with correct column types
- [ ] All 7 pgEnums are defined (`user_role`, `order_status`, `order_priority`, `shipping_company`, `changelog_action`, `order_quality_type`, `inventory_change_type`)
- [ ] Foreign key constraints are set on every reference column with appropriate `onDelete` behavior
- [ ] CHECK constraints enforce: `currentStock >= 0`, `reservedStock >= 0`, `reservedStock <= currentStock`, `quantity > 0`, `shippedQuantity >= 0`
- [ ] Indexes exist on: `orders.status`, `orders.customerId`, `orders.orderNumber`, `products.sku`, `products.categoryId`, `products.barcode`, `orderItems.orderId`, `orderItems.productId`, `inventoryChanges.productId`, `inventoryChanges.timestamp`
- [ ] Drizzle relations are defined for all tables enabling relational queries
- [ ] `drizzle-kit generate` produces valid migration SQL
- [ ] `drizzle-kit migrate` applies cleanly against a fresh database
- [ ] `src/shared/types.ts` exports `$inferSelect` and `$inferInsert` types for every table
- [ ] `npx tsc --noEmit` passes with zero errors
- [ ] A test script can connect, query, and return results from the database
