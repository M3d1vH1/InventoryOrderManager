...
export class DatabaseStorage implements IStorage {
  ...
  async getAllShippingItineraries(): Promise<ShippingItinerary[]> {
    try {
      const result = await this.db.select().from(shippingItineraries).orderBy(shippingItineraries.departureDate);
      return result;
    } catch (error) {
  ...
  async getShippingItinerary(id: number): Promise<ShippingItinerary | undefined> {
    try {
      const result = await this.db.select().from(shippingItineraries).where(eq(shippingItineraries.id, id));
      return result[0];
    } catch (error) {
  ...
  async createShippingItinerary(data: InsertShippingItinerary): Promise<ShippingItinerary> {
    try {
  ...
  async updateShippingItineraryStatus(id: number, status: string): Promise<ShippingItinerary | undefined> {
    try {
  ...
  async getOrdersForItinerary(itineraryId: number): Promise<any[]> {
    try {
  ...
  async addOrderToItinerary(data: { itineraryId: number, orderId: number, boxCount: number, addedById: number }): Promise<void> {
    try {
  ...
  async removeOrderFromItinerary(itineraryId: number, orderId: number): Promise<void> {
    try {
      // Start a transaction to ensure consistency
      await this.db.transaction(async (tx) => {
  ...
  async getUpcomingItineraries(limit = 10): Promise<ShippingItinerary[]> {
    try {
  ...
  async getItinerariesForCalendar(startDate: Date, endDate: Date): Promise<ShippingItinerary[]> {
    try {
      const result = await this.db.select()
        .from(shippingItineraries)
